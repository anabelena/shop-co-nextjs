import Image from "next/image";
import Link from "next/link";
import { IProduct } from "@/types/product";
import RatingStars from "../RatingStars";

export default function ProductCard({
  id,
  title,
  price,
  discountPercentage,
  rating,
  thumbnail,
}: IProduct) {
  return (
    <div className="grid grid-cols-1 gap-y-4  tablet:w-full">
      <div className="rounded-xl border-neutral-200 border-2 cursor-pointer hover:shadow-xl">
        <Link href={`/products/${id}`} key={id}>
          <Image
            className="mx-auto"
            src={thumbnail}
            alt={title}
            width={295}
            height={295}
            style={{ objectFit: "cover" }}
          />
        </Link>
      </div>

      {/* Product Information */}
      <div className="grid grid-cols-1 pl-5 gap-y-2">
        {/* Product Title */}
        <h3 className="text-lg tablet:text-xl font-bold">
          <Link href={`products/${id}`}> {title} </Link>
        </h3>

        {/* Product Rating */}
        <div className="flex items-center gap-1">
          <RatingStars rating={rating} />
          <p className="text-xs tablet:text-sm">
            {rating} <span className="text-neutral-500">/ 5</span>
          </p>
        </div>

        {/* Product Price */}
        <div className="flex items-center font-bold text-lg tablet:text-xl gap-4">
          <span className=" ">{`$${price}`}</span>
          <span className=" line-through text-neutral-600">{0}</span>
          <div className="text-red-500  text-center rounded-xl px-2 text-sm py-1 font-semibold bg-red-200">
            {` ${discountPercentage} %`}
          </div>
        </div>
      </div>
    </div>
  );
}